...
const solution = () => {
    ...
    const calculations = {
        ...
        allDistances: distances.filter((value,index) => index !== 0),
      ...
      while (calculations.customersRemaining > 0) {
        console.log(calculations.customersRemaining)
         ...
         */
         ...
         const skipForTest = false;
          ...
          if (
            ...
            && calculations.customersRemaining && calculations.lastAllocated !== -1) {
            // SOLUTION
            let shortestRoute = maxRouteLength;
            let shortestRouteIndex = -1;
            const sameOriginElementList = calculations.allDistances[calculations.lastAllocated]?.elements;
            for (num in sameOriginElementList) {
              if (num !== calculations.lastAllocated) {
                // console.log(sameOriginElementList[num].distance.value)
                // console.log(shortestRoute)
                if (calculations.prevDistance + sameOriginElementList[num].distance.value <= shortestRoute && calculations.dosfec[num].allocated !== true) {
                  console.log(sameOriginElementList[num].distance.value, num, calculations.lastAllocated)
                  shortestRoute = calculations.prevDistance + sameOriginElementList[num].distance.value;
                  shortestRouteIndex = num;
                }
              }
            }
            if (shortestRouteIndex !== -1 && shortestRoute !== maxRouteLength) {
              const routeObject = {};
              /* Set the customer to the route */
              routeObject.customer = customers[shortestRouteIndex];
              console.log("customer: ", routeObject.customer)
              /* Get the charges for the customer using calculate charges method */
              routeObject.deliveryCharges = calculateCharges(calculations.dosfec[shortestRouteIndex].distance.value);
              routeObject.path = `${sameOriginElementList[shortestRouteIndex].distance.text} from ${customers[calculations.lastAllocated]} to ${customers[shortestRouteIndex]}.`
              calculations.currentRoute.push(routeObject);
              /* Decremtnt the customers remaining to be assigned a route */
              calculations.dosfec[shortestRouteIndex].allocated = true;
              /* Set the route number to this customer */

              calculations.prevDistance = shortestRoute;

              calculations.lastAllocated = shortestRouteIndex;
              console.log(shortestRouteIndex, shortestRoute)
              calculations.customersRemaining -= 1;
            }
            else {
              calculations.lastAllocated = -1;
            }
